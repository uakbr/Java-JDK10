/*
 * @(#)Handler.java	1.8 95/03/23 James Gosling, Jonathan Payne
 * 
...
class Handler extends URLStreamHandler implements Runnable {
    ...
    public InputStream openStream(URL u) {
		...
		try {
		    ...
		    os = new SmtpClient();
		    String D = decodePercent(dest);
		    String pd = u.postData;
		    os.from(user);
		    os.to(D);
		    PrintStream ps = os.startMessage();
		    ps.print("From: "+user+"\nTo: "+D+"\nSubject: Form posted from Mosaic^2\n");
		    ps.print("Content-Type: application/x-www-form-urlencoded\n");
		    int len = pd.length();
		    int pos = 0;
		    int col = 0;
		    ps.print("Content-Length: "+(len+(len/70+1)*2)+"\r\n\r\n");
		    while (pos < len) {
		    ...
		    try { os.closeServer();} catch(Exception e2) {};
		}
	    }
	    else
