...
typedef Hjava_lang_Object Handle;
...
struct methodblock {
    ...
    unsigned short       localvar_table_length;

    bool_t  (*invoker)
      (Handle *o, struct methodblock *mb, int args_size, struct execenv *ee);
    unsigned char args_size;	/* total size of all arguments */
    unsigned char maxstack;	/* maximum stack usage */
    unsigned char nlocals;	/* maximum number of locals */
    union {			/* pointer to the compiled version of this
				  method.  This is unused by the compiler and
				  interpreter, but it's necessary for the
				  machine code generator */
	/* 6 possible return types */
	long	(*I)(struct methodblock *, void *obj, ...);
	int64_t	(*L)(struct methodblock *, void *obj, ...);
	float	(*F)(struct methodblock *, void *obj, ...);
	double	(*D)(struct methodblock *, void *obj, ...);
	HObject*(*H)(struct methodblock *, void *obj, ...);
	void	(*V)(struct methodblock *, void *obj, ...);
    } CompiledCode;
};
...
#define cbPrototype(cb)    ((cb)->prototype)
...
#define CCF_IsResolving	  0x04	/* Resolution is under way */
